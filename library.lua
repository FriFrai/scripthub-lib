...
do 
	...
	do 
		local dropdownOption = {} do 
			...
			dropdownOption.OPTION_HEIGHT = 20 -- Height of each option item

			do
				local instances = {} do 
					local optionButton = Instance.new("TextButton")
					optionButton.Name = "#optionButton"
					optionButton.Size = UDim2.new(1, 0, 0, dropdownOption.OPTION_HEIGHT)
					...
					optionButton.ZIndex = 36 -- Higher than dropdown menu's base

					local padding = Instance.new("UIPadding")
					padding.PaddingLeft = UDim.new(0, 6)
					padding.Parent = optionButton

					instances.optionButton = optionButton
				end
				dropdownOption.instances = instances
			end

			dropdownOption.new = function(self, text, value, parentDropdown)
				local new = setmetatable({}, self)
				new.binds = {}
				new.text = text
				new.value = value
				new.parentDropdown = parentDropdown

				local instances = {}
				instances.optionButton = self.instances.optionButton:Clone()
				instances.optionButton.Text = text
				new.instances = instances

				instances.optionButton.MouseEnter:Connect(function()
					tween(instances.optionButton, { BackgroundColor3 = theme.Button2 }, 0.1, 1)
				end)
				instances.optionButton.MouseLeave:Connect(function()
					tween(instances.optionButton, { BackgroundColor3 = theme.Button1 }, 0.1, 1)
				end)
				instances.optionButton.MouseButton1Click:Connect(function()
					new.parentDropdown:setSelectedOption({text = new.text, value = new.value})
				end)
	...
	do 
		local dropdown = {} do 
			...
			dropdown.DROPDOWN_BUTTON_HEIGHT = 16
			dropdown.DROPDOWN_MENU_ITEM_HEIGHT = elemClasses.dropdownOption.OPTION_HEIGHT
			dropdown.DROPDOWN_MENU_MAX_ITEMS_VISIBLE = 5
			dropdown.DROPDOWN_MENU_PADDING = 4

			do
				local instances = {} do 
					local controlFrame = Instance.new('Frame')
					controlFrame.BackgroundTransparency = 1
					-- controlFrame.BackgroundColor3 = Color3.new(0, 1, 1) -- Debug
					controlFrame.Name = '#control'
					controlFrame.Size = UDim2.new(1, 0, 0, 20) -- Initial height for the button itself
					controlFrame.Visible = true
					controlFrame.ZIndex = 34
					...
					local clickSensor = Instance.new('TextButton') do 
						clickSensor.BackgroundTransparency = 1
						clickSensor.Name = '#click-sensor'
						clickSensor.Size = UDim2.new(1, 0, 0, 20) -- Covers the button area
						clickSensor.Text = ''
						clickSensor.TextTransparency = 1
						clickSensor.ZIndex = 34
						...
						local button = Instance.new('Frame') do 
							button.Active = true
							button.AnchorPoint = Vector2.new(0, 0) -- Easier for layout
							button.BackgroundColor3 = theme.Button1
							button.Name = '#button'
							button.Position = UDim2.fromOffset(3, 2)
							button.Size = UDim2.new(1, -6, 0, dropdown.DROPDOWN_BUTTON_HEIGHT)
							button.Visible = true
							button.ZIndex = 35

							button.Parent = clickSensor -- Parent to clickSensor to ensure it's clickable
							...
							local label = Instance.new('TextLabel') do 
								...
								label.Name = '#label'
								label.RichText = true
								label.Size = UDim2.new(1, -20, 1, 0) -- Make space for icon
								label.Text = 'Select...'
								label.TextColor3 = theme.TextPrimary
								label.TextSize = 14
								label.TextStrokeColor3 = theme.TextStroke
							...
							local icon = Instance.new('ImageLabel') do 
								...
								icon.BackgroundTransparency = 1
								icon.Image = 'rbxassetid://9801473013' -- Down arrow
								icon.ImageColor3 = theme.Secondary -- Or theme.TextPrimary
								icon.Name = '#icon'
								icon.Position = UDim2.new(1, -2, 0.5, 0)
								icon.Rotation = 0
								icon.Size = UDim2.fromOffset(12, 12) -- Smaller icon
								icon.Visible = true
								icon.ZIndex = 35

								icon.Parent = button

								-- No gradient on small icons usually
							end
						end
						...
						local menu = Instance.new('ScrollingFrame') do 
							...
							menu.BorderSizePixel = 0
							menu.ClipsDescendants = true
							menu.CanvasSize = UDim2.fromOffset(0, 0) -- Will be set by UIListLayout
							menu.Name = '#menu'
							menu.Position = UDim2.new(0, 3, 0, 20) -- Position below the button
							menu.ScrollBarImageTransparency = 0.9
							menu.ScrollBarThickness = 3
							menu.ScrollingDirection = 'Y'
							menu.Size = UDim2.new(1, -6, 0, 0) -- Initially hidden (height 0)
							menu.TopImage = 'rbxassetid://9416839567' -- If you want custom scrollbar
							menu.BottomImage = 'rbxassetid://9416839567'
							menu.MidImage = 'rbxassetid://9416839567'
							menu.Visible = false -- Initially hidden
							menu.ZIndex = 36 -- Above button, below tooltips/other windows

							menu.Parent = controlFrame

							local layout = Instance.new('UIListLayout') do 
								layout.Padding = UDim.new(0, 2) -- Small padding between items
								layout.Name = '#layout'
								layout.FillDirection = 'Vertical'
								layout.HorizontalAlignment = 'Center'
								layout.VerticalAlignment = 'Top'
								layout.SortOrder = 'LayoutOrder' -- Keep insertion order

								layout.Parent = menu
							end

							local padding = Instance.new('UIPadding') do 
								padding.PaddingTop = UDim.new(0, dropdown.DROPDOWN_MENU_PADDING / 2)
								padding.PaddingBottom = UDim.new(0, dropdown.DROPDOWN_MENU_PADDING / 2)
								padding.Name = '#padding'
								padding.Parent = menu
							end
			...
			dropdown.open = function(self) 
				...
				local buttonFrame = self.instances.button
				local menuFrame = self.instances.menu
				local controlFrame = self.instances.controlFrame
				local icon = self.instances.icon

				if (self.focused) then
					tween(buttonFrame, {BackgroundColor3 = theme.Button4}, 0.2, 1)
				else
					-- This case might not happen if click makes it lose focus before opening
					tween(buttonFrame, {BackgroundColor3 = theme.Button2}, 0.2, 1) 
				end
				tween(icon, { Rotation = 180, ImageColor3 = theme.Primary }, 0.3, 1)

				menuFrame.Visible = true
				local numItems = #self.options
				local itemsToShow = math.min(numItems, self.DROPDOWN_MENU_MAX_ITEMS_VISIBLE)
				local targetMenuHeight = (itemsToShow * self.DROPDOWN_MENU_ITEM_HEIGHT) + 
					((itemsToShow - 1) * self.instances.menu['#layout'].Padding.Offset) + -- Inter-item padding
					self.DROPDOWN_MENU_PADDING -- Top/Bottom padding of menu

				if numItems == 0 then targetMenuHeight = 0 end

				tween(menuFrame, { Size = UDim2.new(1, -6, 0, targetMenuHeight) }, 0.2, 1)
				tween(controlFrame, { Size = UDim2.new(1, 0, 0, 20 + targetMenuHeight + 2) }, 0.2, 1) -- +2 for spacing
			end
			...
			dropdown.close = function(self) 
				...
				if (self.focused) then
					...
					tween(buttonFrame, {BackgroundColor3 = theme.Button1}, 0.2, 1)
				end
				tween(icon, { Rotation = 0, ImageColor3 = theme.Secondary }, 0.3, 1)

				tween(menuFrame, { Size = UDim2.new(1, -6, 0, 0) }, 0.2, 1).Completed:Connect(function()
					if not self.openState then -- Ensure it's still closed
						menuFrame.Visible = false
					end
				end)
				tween(controlFrame, { Size = UDim2.new(1, 0, 0, 20) }, 0.2, 1)
			end
			...
			dropdown.setSelectedOption = function(self, optionData)
				self.selectedOption = optionData
				self.instances.label.Text = optionData.text
				self:fireEvent("onSelectionChanged", optionData.value, optionData.text)
				self:close()
			end

			dropdown.clearOptions = function(self)
				for _, optData in ipairs(self.options) do
					if optData.instance then
						optData.instance.instances.optionButton:Destroy()
					end
				end
				self.options = {}
				if self.selectedOption then -- Reset selected if it was part of cleared options
					self.selectedOption = nil
					self.instances.label.Text = "Select..." -- Or a placeholder
				end
			end

			dropdown.addOption = function(self, text, value)
				local optionInstance = elemClasses.dropdownOption:new(text, value, self)
				optionInstance.instances.optionButton.Parent = self.instances.menu
				table.insert(self.options, {text = text, value = value, instance = optionInstance})
				return optionInstance
			end

			dropdown.setOptions = function(self, newOptionsTable) 
				self:clearOptions()
				for _, optTbl in ipairs(newOptionsTable) do 
					self:addOption(optTbl.text, optTbl.value)
				end
				-- If there's a default or first item, select it
				if #self.options > 0 and not self.selectedOption then
					-- Attempt to find default or select first
					local defaultVal = self.default
					local foundDefault = false
					if defaultVal ~= nil then
						for _, opt in ipairs(self.options) do
							if opt.value == defaultVal then
								self:setSelectedOption(opt)
								foundDefault = true
								break
							end
						end
					...
					if not foundDefault then
						self:setSelectedOption(self.options[1]) -- Select first if no default or default not found
					end
				elseif #self.options == 0 then
					self.instances.label.Text = "No options"
				end
			end
			...
			dropdown.new = function(self) 
				local new = setmetatable({}, self)
				new.binds = {}
				new.options = {} -- {text, value, instance}
				...
				for i, signals in pairs(self.signals) do 
					local inst = instances[i]
					for signal, func in pairs(signals) do
						local h = inst[signal]:Connect(function(...) -- Pass args for MouseButton1Click
							func(inst, new, ...)
						end)
					end
				end
			...
			elemClasses.section.addDropdown = function(self, settings, callback) 
				...
				local s_title = settings.text or 'Dropdown' -- Placeholder for tooltip
				local s_options = settings.options or {} -- Table of {text = "Display", value = "ActualValue"}
				local s_default = settings.default -- The 'value' to select by default

				local new = dropdown:new()
				new.section = self 
				new.name = s_title -- Used for tooltip
				new.default = s_default -- Store default to apply after options are set
				table.insert(self.controls, new)

				new:setOptions(s_options) -- This will also handle selecting the default or first item

				new.instances.controlFrame.Parent = self.instances.controlMenu

				if typeof(callback) == 'function' then
					new:bindToEvent('onSelectionChanged', callback)
				end
				return new
			end
		end
	...
	do
		local multidropdownOption = {} do 
			multidropdownOption.__index = multidropdownOption 
			setmetatable(multidropdownOption, elemClasses.baseElement)

			multidropdownOption.class = 'multidropdownOption'
			multidropdownOption.OPTION_HEIGHT = 20
			multidropdownOption.isSelected = false

			do
				local instances = {} do 
					local optionButton = Instance.new("TextButton")
					optionButton.Name = "#optionButton"
					optionButton.Size = UDim2.new(1, 0, 0, multidropdownOption.OPTION_HEIGHT)
					optionButton.BackgroundColor3 = theme.Button1
					optionButton.BorderSizePixel = 0
					optionButton.AutoButtonColor = false
					optionButton.Font = "SourceSans"
				...
				multidropdownOption.instances = instances
			end

			multidropdownOption.setSelected = function(self, state)
				if self.isSelected == state then return end
				self.isSelected = state
				
				local targetColor = state and theme.Primary or theme.TextPrimary
				tween(self.instances.optionButton, { TextColor3 = targetColor }, 0.2, 1)
			end

			multidropdownOption.new = function(self, text, value, parentDropdown)
				local new = setmetatable({}, self)
				new.text = text
				new.value = value
				new.parentDropdown = parentDropdown
				...
				instances.optionButton.MouseButton1Click:Connect(function()
					new.parentDropdown:toggleValue(new.value)
				end)

				return new
			end
		end
		elemClasses.multidropdownOption = multidropdownOption
	end
	...
	do 
		local multidropdown = {} do 
			...
			-- Herda constantes do dropdown normal para consistência
			multidropdown.DROPDOWN_BUTTON_HEIGHT = elemClasses.dropdown.DROPDOWN_BUTTON_HEIGHT
			multidropdown.DROPDOWN_MENU_ITEM_HEIGHT = elemClasses.multidropdownOption.OPTION_HEIGHT
			multidropdown.DROPDOWN_MENU_MAX_ITEMS_VISIBLE = elemClasses.dropdown.DROPDOWN_MENU_MAX_ITEMS_VISIBLE
			multidropdown.DROPDOWN_MENU_PADDING = elemClasses.dropdown.DROPDOWN_MENU_PADDING
			
			-- Clona a estrutura de instâncias do dropdown para manter o visual
			multidropdown.instances = elemClasses.dropdown.instances

			multidropdown.focused = false
			multidropdown.openState = false

			multidropdown.updateLabelText = function(self)
				local count = 0
				for _ in pairs(self.selectedValues) do
					count += 1
				end

				if count == 0 then
					self.instances.label.Text = "None selected"
				elseif count == 1 then
					self.instances.label.Text = "1 selected"
				else
					self.instances.label.Text = tostring(count) .. " selected"
				end
			end
			
			multidropdown.toggleValue = function(self, value)
				local option = self.optionsByValue[value]
				if not option then return end

				if self.selectedValues[value] then
					self.selectedValues[value] = nil -- Desseleciona
					option:setSelected(false)
				else
					self.selectedValues[value] = true -- Seleciona
					option:setSelected(true)
				end
				
				self:updateLabelText()
				self:fireEvent("onSelectionChanged", self:getValues())
			end
			
			multidropdown.getValues = function(self)
				local values = {}
				for v in pairs(self.selectedValues) do
					table.insert(values, v)
				end
				return values
			end

			multidropdown.setOptions = function(self, newOptionsTable)
				-- Limpa opções antigas da UI
				for _, opt in ipairs(self.options) do
					opt.instances.optionButton:Destroy()
				end
				self.options = {}
				self.optionsByValue = {}
				...
				for _, optTbl in ipairs(newOptionsTable) do
					local optionInstance = elemClasses.multidropdownOption:new(optTbl.text, optTbl.value, self)
					optionInstance.instances.optionButton.Parent = self.instances.menu
					table.insert(self.options, optionInstance)
					self.optionsByValue[optTbl.value] = optionInstance
				end

				-- Aplica seleção (incluindo a padrão)
				for value, option in pairs(self.optionsByValue) do
					option:setSelected(self.selectedValues[value] == true)
				end

				self:updateLabelText()
			end

			-- Reutiliza open/close e click do dropdown normal, pois a lógica é a mesma
			multidropdown.open = elemClasses.dropdown.open
			multidropdown.close = elemClasses.dropdown.close
			multidropdown.click = elemClasses.dropdown.click
			multidropdown.signals = elemClasses.dropdown.signals
			...
			elemClasses.section.addMultidropdown = function(self, settings, callback) 
				...
				local s_title = settings.text or 'Multidropdown'
				local s_options = settings.options or {}
				local s_defaults = settings.defaults or {} -- Note o 's' em defaults
				...
				-- Define os valores padrão antes de adicionar as opções
				for _, val in ipairs(s_defaults) do
					new.selectedValues[val] = true
				end
  	...
  	-- TOGGLED DROPDOWN OPTION (Corrected for ZIndex)
	do
		local toggledDropdownOption = {} do
			toggledDropdownOption.__index = toggledDropdownOption
			setmetatable(toggledDropdownOption, elemClasses.baseElement)

			toggledDropdownOption.class = 'toggledDropdownOption'
			toggledDropdownOption.isToggled = false
			toggledDropdownOption.OPTION_HEIGHT = 20

			do
				local instances = {} do 
					local controlFrame = elemClasses.toggle.instances.controlFrame:Clone()
					controlFrame.Size = UDim2.new(1, -8, 0, toggledDropdownOption.OPTION_HEIGHT)
					controlFrame.Position = UDim2.fromOffset(4, 0)
					instances.controlFrame = controlFrame
				end
				toggledDropdownOption.instances = instances
			end

			toggledDropdownOption.toggle = function(self)
				local newState = not self.isToggled
				self.isToggled = newState
				local toggleFrame = self.instances.toggle
				local icon = toggleFrame['#icon']
				local isFocused = self.parentDropdown.focused

				if newState then
					icon.Image = 'rbxassetid://9801457539'
					tween(icon, { Rotation = 0, ImageColor3 = theme.Primary }, 0.3, 1)
					tween(toggleFrame, { BackgroundColor3 = isFocused and theme.Button4 or theme.Button3 }, 0.2, 1)
				else
					icon.Image = 'rbxassetid://9801456486'
					tween(icon, { Rotation = 360, ImageColor3 = theme.Secondary }, 0.3, 1)
					tween(toggleFrame, { BackgroundColor3 = isFocused and theme.Button2 or theme.Button1 }, 0.2, 1)
				end
				
				self.parentDropdown:onOptionToggled(self.value, newState)
			end

			toggledDropdownOption.new = function(self, text, value, initialState, parentDropdown)
				local new = setmetatable({}, self)
				new.text, new.value, new.parentDropdown = text, value, parentDropdown

				local instances = {}
				instances.controlFrame = self.instances.controlFrame:Clone()
				
				-- [[ FIX: Aumenta o ZIndex para ficar acima do menu (ZIndex 36) ]]
				instances.controlFrame.ZIndex = 37
				for _, child in ipairs(instances.controlFrame:GetDescendants()) do
					if child:IsA("GuiObject") then
						child.ZIndex = child.ZIndex + 3 -- Garante que os filhos também fiquem acima
					end
				end
				...
				instances.label.Text = text
				instances.backToggle.MouseButton1Click:Connect(function() new:toggle() end)
				
				if initialState then
					new.isToggled = true
					instances.toggle.BackgroundColor3 = theme.Button3
					instances.toggle['#icon'].Image = 'rbxassetid://9801457539'
					instances.toggle['#icon'].ImageColor3 = theme.Primary
					instances.toggle['#icon'].Rotation = 0
				end
				
				return new
			end
		end
		elemClasses.toggledDropdownOption = toggledDropdownOption
	end

	-- TOGGLED DROPDOWN (Corrected)
	do
		local toggledDropdown = {} do
			toggledDropdown.__index = toggledDropdown
			setmetatable(toggledDropdown, elemClasses.baseElement)

			toggledDropdown.class = 'toggledDropdown'
			toggledDropdown.instances = elemClasses.dropdown.instances
			toggledDropdown.openState = false
			toggledDropdown.focused = false

			-- [[ FIX: Adiciona TODAS as constantes necessárias para o cálculo de altura ]]
			toggledDropdown.DROPDOWN_MENU_ITEM_HEIGHT = elemClasses.toggledDropdownOption.OPTION_HEIGHT
			toggledDropdown.DROPDOWN_MENU_MAX_ITEMS_VISIBLE = 5
			toggledDropdown.DROPDOWN_MENU_PADDING = 4
			
			toggledDropdown.updateLabelText = function(self)
				local count = 0
				for _, state in pairs(self.toggledStates) do if state then count = count + 1 end end
				if count == 0 then self.instances.label.Text = "None enabled"
				elseif count == 1 then self.instances.label.Text = "1 enabled"
				else self.instances.label.Text = tostring(count) .. " enabled" end
			end

			toggledDropdown.onOptionToggled = function(self, value, newState)
				self.toggledStates[value] = newState
				self:updateLabelText()
				self:fireEvent("onStateChanged", self:getToggled())
			end

			toggledDropdown.getToggled = function(self)
				local toggled = {}
				for value, state in pairs(self.toggledStates) do if state then table.insert(toggled, value) end end
				return toggled
			end

			toggledDropdown.setOptions = function(self, optionsTable)
				for _, opt in ipairs(self.options or {}) do opt.instances.controlFrame:Destroy() end
				self.options, self.toggledStates = {}, {}
				for _, settings in ipairs(optionsTable) do
					local text, value, state = settings.text, settings.value, settings.state or false
					self.toggledStates[value] = state
					local optionInstance = elemClasses.toggledDropdownOption:new(text, value, state, self)
					optionInstance.instances.controlFrame.Parent = self.instances.menu
					table.insert(self.options, optionInstance)
				end
				self:updateLabelText()
			end
			
			-- Reutiliza a lógica de abrir/fechar/clicar que agora funcionará corretamente
			toggledDropdown.open = elemClasses.dropdown.open
			toggledDropdown.close = elemClasses.dropdown.close
			toggledDropdown.click = elemClasses.dropdown.click
			toggledDropdown.signals = elemClasses.dropdown.signals
			toggledDropdown.__hotkeyFunc = toggledDropdown.click

			toggledDropdown.new = function(self)
				local new = setmetatable({}, self)
				new.binds, new.options, new.toggledStates = {}, {}, {}
				local instances = {}
				instances.controlFrame = self.instances.controlFrame:Clone()
				instances.clickSensor = instances.controlFrame['#click-sensor']
				instances.button = instances.clickSensor['#button']
				...
				for i, signals in pairs(self.signals) do
					local inst = instances[i]
					for signal, func in pairs(signals) do inst[signal]:Connect(function(...) func(inst, new, ...) end) end
				end
				return new
			end

			elemClasses.section.addToggledDropdown = function(self, settings, callback)
				if (not typeof(settings) == 'table') then return error('expected table for settings', 2) end
				local new = toggledDropdown:new()
				new.section = self
				new.name = settings.text or 'Toggled Dropdown'
				table.insert(self.controls, new)
				new:setOptions(settings.options or {})
				new.instances.controlFrame.Parent = self.instances.controlMenu
				if typeof(callback) == 'function' then new:bindToEvent('onStateChanged', callback) end
				return new
			end
		end
		elemClasses.toggledDropdown = toggledDropdown
	end
